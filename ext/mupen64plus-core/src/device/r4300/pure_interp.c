...
#define DECLARE_JUMP(name, destination, condition, link, likely, cop1, extra) \
...
static uint32_t InterpretOpcodeX(struct r4300_core* r4300)
{
	...
	uint32_t pre_pc = r4300->interp_PC.addr;


	trainer_insn_pre(reality_state, pre_pc);
...
{
	trainer_insn_post(reality_state, InterpretOpcodeX(r4300));
}
