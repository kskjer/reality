...
m64p_error main_run(void)
{
	...
	reality_state = trainer_init(
		mem_base_u32(g_mem_base, MM_RDRAM_DRAM),
		mem_base_u32(g_mem_base, MM_CART_ROM), 
		(uint32_t)g_rom_size,
		g_dev.r4300.regs,
		g_dev.r4300.cp1.regs,
		g_dev.r4300.cp0.regs
	);

	printf("Reality trainer initialized. state: %p\n", reality_state);

    poweron_device(&g_dev);
    pif_bootrom_hle_execute(&g_dev.r4300);
    run_device(&g_dev);

	trainer_deinit(&reality_state);
